...
const state = proxy({
  ...
  openUrls: [1, 2, 3, 4, 5, 6, 7].map((u) => `/works/mobile/${u}_open_mobile.jpg`)
})
...
function Item({ index, position, scale, c = new THREE.Color(), ...props }) {
  ...
  useFrame((state, delta) => {
    const y = scroll.curve(index / urls.length - 1.5 / urls.length, 4 / urls.length)
    ref.current.material.scale[1] = ref.current.scale.y = damp(ref.current.scale.y, clicked === index ? 6 : 4 + y, 8, delta)
    ref.current.material.scale[0] = ref.current.scale.x = damp(ref.current.scale.x, clicked === index ? 3 : scale[0], 6, delta)
    ...
    ref.current.material.color.lerp(c.set(hovered || clicked === index ? 'white' : '#F4F4F4'), hovered ? 0.3 : 0.1)
  })
  return <Image ref={ref} {...props} position={position} scale={scale} onClick={click} onPointerOver={over} onPointerOut={out} url={clicked === index ? openUrls[index] : urls[index]}
/>
}
